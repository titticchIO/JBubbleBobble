...
import editor.controller.NewLevelActionListener;
	...
	public LevelsPopUpMenu(MenuType menuType, EditorFrame editorFrame) {
		...
		levels.forEach(level -> {
			...
			button.addActionListener(e -> {
				...
				LevelsPopUpMenu.this.setVisible(false); // Nasconde il popup dopo l'azione
			});
			panel.add(button);
		});

		if (menuType == MenuType.SAVE)
			panel.add(new JButton("new Level (" + (levels.size() + 1) + ")") {
				{
					addActionListener(new NewLevelActionListener());
				}
			});
