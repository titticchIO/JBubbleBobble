...
public class SkelMonsta extends Enemy {
	...
	public void bounce() {
		...
		/*
		 * // Controlla se il nemico ha raggiunto i bordi superiore o inferiore del
		 * gioco if ((y <= Settings.TILE_SIZE && HelpMethods.isSolidHorizontalLine(x, x
		 * + width, y - 1)) || (y + height >= Settings.GAME_HEIGHT - Settings.TILE_SIZE
		 * && HelpMethods.isSolidHorizontalLine(x, x + width, y + height + 1))) {
		 * System.out.println("siuuum sotto"); // Inverti la direzione verticale e
		 * applica un elemento casuale alla velocità airSpeed = -airSpeed * (0.8f +
		 * random.nextFloat() * 0.4f); // Velocità tra 80% e 120% dell'attuale //
		 * Assicurati che la velocità non sia inferiore a MIN_SPEED o superiore a
		 * MAX_SPEED airSpeed = Math.max(Math.min(airSpeed, MAX_SPEED), -MAX_SPEED); if
		 * (Math.abs(airSpeed) < MIN_SPEED) { airSpeed = Math.signum(airSpeed) *
		 * MIN_SPEED; } }
		 */
